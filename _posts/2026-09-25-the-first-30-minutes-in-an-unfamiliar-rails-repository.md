...
On a Mac with compatible Ruby, Rails and PostgreSQL installed, follow `caseflow-overlay/README.md` in the [companion materials](/assets/downloads/first-30-minutes-rails-repository-support.zip). Copy the overlay into a newly generated Rails API shell, inspect the diff, add a compatible `rspec-rails`, and record the actual output of:
...
- [Download the CaseFlow companion package](/assets/downloads/first-30-minutes-rails-repository-support.zip): source overlay, `docs/labs/01-codebase-diagnosis.md`, `prompts/01-trace-request.md`, Mermaid source, answer guide and validation status. The overlay is not a complete runnable app; its request specs remain unexecuted in this authoring environment.
